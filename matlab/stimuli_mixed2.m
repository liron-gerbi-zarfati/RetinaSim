...
phos=fspecial('gaussian', 20, 20^0.5);
phos=phos-min(min(phos));
phos=phos/max(max(phos));
...
imgs1=imgs(:,:,[2:4:16]);
stimType(imgsCount,1:4)=[12,0,max(max(mask.*phosphene_image))./max(max(phosphene_image)),directioni]; %  chupchik size, shift, luminosity, direction
